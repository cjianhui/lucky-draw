(function ($, document, machine) {
  const Item = ({ text, onDelete }) => (
    <li key={Math.random()} id={text}>
      {text}
      <span className="delete" title="Delete" onClick={() => onDelete(text)}>
    ...
    return (
      <ul className="item-list">
        {items.map((itemText, i) => (
          <Item text={itemText} key={i} onDelete={onDelete} />
        ))}
      </ul>
    );
  };
  ...
  class InputForm extends React.Component {
    constructor(props) {
      ...
      this.handleChangeNumberOfDraws = this.handleChangeNumberOfDraws.bind(
        this
      );
          ...
          this.setState(
            ...
            () => {
              ...
              machine.onResultChange((poorMan) => {
                ...
                $("#result-view-container").addClass(
    ...
    setWithoutReplacement() {
      machine.setSettings({
        isWithoutReplacement: $("#rand-without-replacement").is(":checked"),
      });
    }
          ...
          <form id="edit-item-form" onSubmit={this.handleAdd}>
            ...
            <div className="item-list-container">
              ...
              <div style={{ marginBottom: 16, marginTop: 16 }}>
                <label className={"block"} style={{ marginBottom: 2 }}>
                  Number Of Draws per batch
                </label>
                <input
                  value={this.state.numberOfDraws}
                  type="number"
                  placeholder="Number Of Draws"
                  id="number-of-draws"
                  onChange={this.handleChangeNumberOfDraws}
                  min={1}
                  max={Math.max(this.state.items.length, 1)}
                />
              </div>
              <div style={{ marginBottom: 16, marginTop: 16 }}>
                <label className={"block"} style={{ marginBottom: 2 }}>
                ...
                <input
                  ...
                  type="number"
                  placeholder="Font Size (in pixel)"
                  id="font-size"
              ...
              <label htmlFor="rand-without-replacement" className="text-left">
                <input
                  checked={!!this.state.isWithoutReplacement}
                  onChange={this.setWithoutReplacement}
                  type="checkbox"
                  id="rand-without-replacement"
            ...
            <div className="btn-set">
              <button
                className="btn primary-btn btn-done"
                onClick={this.handleInputDone}
              >
                Done
              </button>
            </div>
